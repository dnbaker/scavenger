...
def parse_als_file_set(folder):
    ...
    all_features = sorted(
        reduce(lambda x, y: x | y, (x[0] for x in feature_and_samples)))
    ...
    assert all(x.shape[0] == len(samples)
               for x, samples in zip(submats, individual_sample_lists))
    maxv = max(map(np.max, submats))
    total_map = sp.vstack(submats, dtype=size2intype(maxv))
    coords = np.array([list(map(float, x.split("_")[-2:]))
...
def parse_aquila_folder(path):
    ...
    markers = list(map(str.strip, gzip.open(
        path + "/markers_names.csv.gz", "rt")))[1:]
...
__all__ = ["parse_als_st_matrix", "als_st_features",
           "parse_als_file_set", "size2intype"]
